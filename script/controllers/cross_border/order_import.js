define(['app'], function(app) {
    ...
    return app.controller('OrderImportCtrl', ['$scope', 'OrderImportService', '$log', '$timeout', 'oms', function($scope, service, $log, $timeout, oms) {
        var vm = $scope;
        ...
        vm.status = {
            ...
            import: function() {
                return (
                    vm.status.uploading ||
                    !paras.file ||
                    !paras.condition.hgAreaCode ||
                    !paras.condition.customer ||
        ...
        function getTemplateByArea() {
            var temp = 'templates/%sOrderImportTemplate.xls';

            var codes = { 'NBC': 'nb', 'CQC': 'cq' , 'DGC': 'st' };
            var hg = paras.condition.hgAreaCode;
            if (hg && config.area_hash) {
                code = codes[config.area_hash[hg].raw['customCode']];
                if (code) {
